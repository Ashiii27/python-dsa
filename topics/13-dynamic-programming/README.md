...
|---|---|---|---|---|
| Climbing Stairs | Easy | dp[i]=dp[i-1]+dp[i-2]. | O(n) | O(1) |
| House Robber | Medium | At each house choose rob or skip. | O(n) | O(1) |
| Coin Change | Medium | dp[amount] is min coins to form amount. | O(amount*coins) | O(amount) |
| Longest Increasing Subsequence | Medium | Patience tails array with binary search. | O(n log n) | O(n) |
| Longest Common Subsequence | Medium | 2D DP over prefixes. | O(nm) | O(nm) |
| Edit Distance | Hard | Insert/delete/replace transition over prefixes. | O(nm) | O(nm) |
| Word Break | Medium | dp[i] true if a valid word ends at i. | O(n^2) | O(n) |
| Decode Ways | Medium | Ways to decode prefix i from one/two-digit endings. | O(n) | O(1) |
| Partition Equal Subset Sum | Medium | 0/1 knapsack for target sum/2. | O(n*target) | O(target) |
| Burst Balloons | Hard | Interval DP choosing last balloon in interval. | O(n^3) | O(n^2) |
| Regular Expression Matching | Hard | String DP with dot and star transitions. | O(nm) | O(nm) |
